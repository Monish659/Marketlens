...
export function LoginForm({
  ...
  const handleSocialLogin = async (connection: string) => {
    if (connection !== 'google-oauth2') {
      alert('Apple login is not configured yet. Please use Google or email/password for now.')
      return
    }

    setIsLoading(true)
    try {
      // Clear any stale state before redirecting to Google OAuth
      localStorage.removeItem('auth_tokens')
      localStorage.removeItem('user_data')
      await startGoogleOAuth()
    } catch (error) {
      console.error('❌ Google login error:', error)
      alert('Google login failed. Check Supabase Google provider settings and try again.')
      setIsLoading(false)
    }
  }
